...
const SignupPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response && response.id) {
        setAlertMessage("Registration successful! Please check your email for activation.")
        toast({
          variant: "success",
          title:
  ...
  return (
    <section >
      ...
      {alertMessage && <div className=" text-center flex justify-center inset-80 fixed z-50 h-fit">
        <div className="border p-3 shadow-md border-green-600 bg-green-300">
        {alertMessage} 
        </div>
      </div>}
      <div className=""></div>
        ...
        <div className="border mx-5 p-10 shadow-lg md:w-full">
          ...
          <form className="flex flex-col gap-y-4 mt-3" onSubmit={handleSubmit}>
            ...
            <div className="">
              <CustomBtn
                btnType="submit"
                btnStyles="bg-blue-500 p-2 rounded-md text-center text-white hover:bg-blue-700 w-full"
                btnName="Sign Up"
              />
            </div>
          </form>
